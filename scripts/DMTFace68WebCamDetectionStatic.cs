...
namespace DlibFaceLandmarkDetectorExample
{
    ...
    /// </summary>
    public class DMTFace68WebCamDetectionStatic : MonoBehaviour
    {
        // Constants
        ...
        private async void Start()
        {
            ...
            DMT.DMTStaticStoreFace68.imgWidth = RequestedWidth;
            DMT.DMTStaticStoreFace68.imgHeight = RequestedHeight;
            Debug.Log("DMTFace68WebCamDetectionStatic::Start Image>" + DMT.DMTStaticStoreFace68.imgWidth + "x" + DMT.DMTStaticStoreFace68.imgHeight);
        ...
        private void Update()
        {
            ...
            if (_webCamTexture.isPlaying && _webCamTexture.didUpdateThisFrame)
            {
                ...
                if (colors != null)
                {
                    // NIS - main CS FaceLandmarkDetector.cs
                    // C:\Data\01_dev\2026_dmt4_openCV\00_opencv_startup\Assets\DlibFaceLandmarkDetector\Scripts
                    // 708 public List<Vector2> DetectLandmark(double left, double top, double width, double height) 

                    _faceLandmarkDetector.SetImage<Color32>(colors, _texture.width, _texture.height, 4, true);
                    List<UnityEngine.Rect> detectResult = _faceLandmarkDetector.Detect(); // or OpenCVCoreModule

                    // NIS: detected Faces
                    DMT.DMTStaticStoreFace68.faceCounter = detectResult.Count;

                    List<Vector2> detectedLandmarks = new List<Vector2>();
                    foreach (var rect in detectResult)
                    {
                        detectedLandmarks = _faceLandmarkDetector.DetectLandmark(rect);
                        // NIS: detected landmarks for one face
                        DMT.DMTStaticStoreFace68.faceLandmark = detectedLandmarks;
